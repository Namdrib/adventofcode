...
public class day18 {
   ...
   * <li><code>mul X Y</code> sets register X to the result of multiplying the value contained in
   * register X by the value of Y.
   * <li><code>mod X Y</code> sets register X to the remainder of dividing the value contained in
   * register X by the value of Y (that is, it sets X to the result of X modulo Y).
   * <li><code>rcv X</code> recovers the frequency of the last sound played, but only when the value
   * of X is not zero. (If it is zero, the command does nothing.)
   * <li><code>jgz X Y</code> jumps with an offset of the value of Y, but only if the value of X is
  ...
  private void processInstructions(List<String> instructions) {
    int count = 0;
    insLoop: for (int i = 0; i < instructions.size(); i++, count++) {
      // System.out.println("count " + String.format("%0 4d", count) + ", ins " + String.format("%
      // 03d", i) + ": " + instructions.get(i));
